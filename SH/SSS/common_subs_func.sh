#!/bin/sh
###############################################################################
# $Id: common_subs_func.sh,v 1.13 2015/02/17 09:46:18 cvsdzmitry Exp $'
###############################################################################
#
# common_subs_func.sh
...
# 29/05/15 - US - -ww option is added to $PSARGS for Linux
# 19/06/15 - US - exit code value is fixed
...
Create_Empty_Log() {
	...
	if [ "$SYST_ID" ];then
		SYST_ID=`echo $SYST_ID | cut -c1-8`
	fi
    echo "COMPUTER_SYS_ID=$COMPUTER_SYS_ID;CUST_ID=;SYST_ID=$SYST_ID;HOSTNAME=$HOSTNAME;SUBSYSTEM_INSTANCE=NO;SUBSYSTEM_TYPE=$2;MW_VERSION=;MW_EDITION=;FIXPACK=;INSTANCE_PATH=;DB_NAME=;SCAN_TIME=$SCAN_TIME;SAP_INSTALLED=;SAP_CVERS=;OS_VERSION=$osver;INSTANCE_PORT=;NB_TABLES=;NB_INDEXES=;ALLOC_DB=;USED_DB=;ALLOC_LOG=;USED_LOG=;DB_PART=;TABLE_PART=;INDEX_PART=;SERVER_TYPE=$ostype;DB_TYPE=;DBMS_TYPE=;SDC=${SSS_TOOL};DB_USAGE=;SVC_OFFERED=;HC_REQUIRED=;MW_INST_ID=;MW_MODULE=;MW_NB_EAR=;MW_NB_USERS=;MW_NB_MNDTS=;SW_BUNDLE=;SCANNER_VERSION=$SCANNER_VERSION;" > $1
}
...
Create_NotSupported_Log() {
    ...
    echo "COMPUTER_SYS_ID=$COMPUTER_SYS_ID;CUST_ID=;SYST_ID=$SYST_ID;HOSTNAME=$HOSTNAME;SUBSYSTEM_INSTANCE=NO;SUBSYSTEM_TYPE=$2;MW_VERSION=;MW_EDITION=O.S. Not supported;FIXPACK=;INSTANCE_PATH=;DB_NAME=;SCAN_TIME=$SCAN_TIME;SAP_INSTALLED=;SAP_CVERS=;OS_VERSION=$osver;INSTANCE_PORT=;NB_TABLES=;NB_INDEXES=;ALLOC_DB=;USED_DB=;ALLOC_LOG=;USED_LOG=;DB_PART=;TABLE_PART=;INDEX_PART=;SERVER_TYPE=$ostype;DB_TYPE=;DBMS_TYPE=;SDC=${SSS_TOOL};DB_USAGE=;SVC_OFFERED=;HC_REQUIRED=;MW_INST_ID=;MW_MODULE=;MW_NB_EAR=;MW_NB_USERS=;MW_NB_MNDTS=;SW_BUNDLE=;SCANNER_VERSION=$SCANNER_VERSION;" > $1
}
...
Detect_Host_Parameters() {
 ...
 Show_PS_Output() {
  	if [ "$DEBUG_MODE" = "true" ]; then
 		if [ "$UOS" = "LINUX" ] && (echo "$PSARGS" | grep -q -- "-ww"); then
 			set +x
	    	echo "-------------------------------------- START PS -WW -EF ---------------------------------------"
 			set -x
			ps -ww -ef
