...
chown ora11g:oinstall -R /opt/oracle_2
...
while [  $CONTADOR -lt $LINEAS ]; 
	...
	OHOME=`awk -v lin="$LIN" ' NR == lin ' lineas.log | awk -F "|" '{print $1}'`
	...
	case "$VERSION" in
		...
		pause 'Pulsar [Enter] para continuar...'
		clone $VORACLE_HOME $VORACLE_BASE
		pause 'Pulsar [Enter] para continuar...'
		resp_file $VORACLE_HOME /opt/oracle_2/admin/work/Patch_PSU/file.rsp
		pause 'Pulsar [Enter] para continuar...'
		fopatch p19769496 $VORACLE_HOME /opt/oracle_2/admin/work/Patch_PSU/file.rsp $DBVERSION $OSARCH
		pause 'Pulsar [Enter] para continuar...'
		else
		directory 11.2.0.3 $VORACLE_BASE
		pause 'Pulsar [Enter] para continuar...'
		...
		pause 'Pulsar [Enter] para continuar...'
		clone $VORACLE_HOME $ORACLE_BASE
		pause 'Pulsar [Enter] para continuar...'
		resp_file $VORACLE_HOME /opt/oracle_2/admin/work/Patch_PSU/file.rsp
		pause 'Pulsar [Enter] para continuar...'
		fopatch p19769496 $VORACLE_HOME /opt/oracle_2/admin/work/Patch_PSU/file.rsp $DBVERSION $OSARCH
		...
		resp_file $VORACLE_HOME /opt/oracle_2/admin/work/Patch_PSU/file.rsp
		pause 'Pulsar [Enter] para continuar...'
		fopatch p19769489 $VORACLE_HOME /opt/oracle_2/admin/work/Patch_PSU/file.rsp $DBVERSION $OSARCH
		pause 'Pulsar [Enter] para continuar...'
		else
		directory 11.2.0.4 $VORACLE_BASE
		pause 'Pulsar [Enter] para continuar...'
